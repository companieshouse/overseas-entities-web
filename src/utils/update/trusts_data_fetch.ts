...
export const retrieveTrustsData = (appData: ApplicationData) => {
  if (!hasFetchedTrustsData(appData)) {
    initialiseTrustsUpdateAppData(appData);
    if (appData.update) {
      retrieveTrusts(appData);
      appData.update.trust_data_fetched = true;
    }
  }
};

const hasFetchedTrustsData = (appData: ApplicationData) => appData?.update?.trust_data_fetched ?? false;

const initialiseTrustsUpdateAppData = (appData: ApplicationData) => {
  if (!appData.update) {
    appData.update = {};
  }
...
const retrieveTrusts = (appData: ApplicationData) => {
  if (!appData.update) {
    appData.update = {};
  }
  ...
  appData.update.review_trusts = [{
    ...
    HISTORICAL_BO: [
      {
        id: "411fedde-ed76-4f58-8c19-0ce4a80b4956",
