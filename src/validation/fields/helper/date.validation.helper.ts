...
export const dateValidations = (dateContext: dateContext, min: number, max: number) => {
  return [
    body(dateContext.day.name)
      ...
      .isLength({ min: 1, max: 2 }).withMessage(dateContext.day.errMsg[1] || ErrorMessages.DAY_LENGTH),
    body(dateContext.month.name)
      ...
      .isLength({ min: 1, max: 2 }).withMessage(dateContext.month.errMsg[1] || ErrorMessages.MONTH_LENGTH),
