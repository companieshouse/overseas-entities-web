...
describe('Update - Manage Trusts - Orchestrator', () => {
  ...
  test.each([
    ['GET', get],
    ['POST', post],
  ])('%s - when no trusts in review, but there is a trust to review, updates app data and redirects to review the trust page', async (_, handler) => {
    const appData: ApplicationData = createAppData({ reviewTrusts: [{ review: { in_review: false } }] });
    mockGetApplicationData.mockReturnValue(appData);
    ...
    expect((appData.update?.review_trusts ?? [])[0].review?.in_review).toBe(true);
