...
export const post = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res.redirect(`${UPDATE_MANAGE_TRUSTS_TELL_US_ABOUT_THE_INDIVIDUAL_URL}${TRUST_ID}`);
  } catch (error) {
    next(error);
  }
};
...
export const postSubmit = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    setIndividualsAsReviewed(appData, trustId);
...
const setIndividualsAsReviewed = (appData: ApplicationData, trustId: string) => {
  const trust = (appData.update?.review_trusts ?? []).find(trust => trust.trust_id === trustId);

  if (!trust?.review) { return; }

  trust.review.reviewed_individuals = true;
};
