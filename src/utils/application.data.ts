...
export const allBeneficialOwners = (appData: ApplicationData): Array<BeneficialOwnerIndividual | BeneficialOwnerCorporate | BeneficialOwnerGov> => {
  if (!appData.update?.no_change) {
    return (appData.beneficial_owners_individual?.filter(relevant => !relevant.relevant_period) ?? [])
      .concat(
        appData.beneficial_owners_government_or_public_authority?.filter(relevant => !relevant.relevant_period) ?? [],
        appData.beneficial_owners_corporate?.filter(relevant => !relevant.relevant_period) ?? []);
  } else {
    return (appData.update.review_beneficial_owners_individual?.filter(relevant => !relevant.relevant_period) ?? [])
      .concat(
        appData.update.review_beneficial_owners_government_or_public_authority?.filter(relevant => !relevant.relevant_period) ?? [],
        appData.update.review_beneficial_owners_corporate?.filter(relevant => !relevant.relevant_period) ?? []);
  }
};
