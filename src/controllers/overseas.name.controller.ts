...
export const post = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (isActiveFeature(config.FEATURE_FLAG_ENABLE_SAVE_AND_RESUME_17102022)) {
      ...
      nextPageUrl = getUrlWithTransactionIdAndOverseasEntityId(config.PRESENTER_PARAMS_URL, appData[Transactionkey] as string, appData[OverseasEntityKey] as string);
    }
