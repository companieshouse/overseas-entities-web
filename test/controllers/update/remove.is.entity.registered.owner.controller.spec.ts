...
describe("Remove registered owner controller", () => {
  ...
  describe("GET tests", () => {
    test(`renders the ${config.REMOVE_IS_ENTITY_REGISTERED_OWNER_PAGE} page`, async () => {
      ...
      expect(resp.text).toContain(`${config.UPDATE_AN_OVERSEAS_ENTITY_URL}sign-out?page=${config.REMOVE_SECTION}${config.REMOVE_IS_ENTITY_REGISTERED_OWNER_PAGE}&amp;${config.JOURNEY_QUERY_PARAM}=${config.JourneyType.remove}`);
