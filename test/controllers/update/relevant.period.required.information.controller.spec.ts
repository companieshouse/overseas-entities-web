...
describe("relevant period required information page tests", () => {
  ...
  describe("POST tests", () => {
    test.skip(`renders the ${config.RELEVANT_PERIOD_REQUIRED_INFORMATION_CONFIRM_URL} page when yes is selected`, async () => {
    ...
    test.skip("catch error when posting the page", async () => {
      mockLoggerDebugRequest.mockImplementationOnce( () => { throw new Error(ANY_MESSAGE_ERROR); });
      const resp = await request(app)
