...
describe('Update - Trusts - Individuals or entities involved', () => {
  ...
  describe('POST tests', () => {
    ...
    test('should error if relevant period beneficiary if ceased date before create date', async () => {
      const formSubmission = {
        trusteeId: 'trustee-id-2',
      ...
      const resp = await request(app)
        .post(UPDATE_TRUSTS_INDIVIDUALS_OR_ENTITIES_INVOLVED_URL + "/" + trustId + TRUST_INDIVIDUAL_BENEFICIAL_OWNER_URL + RELEVANT_PERIOD_QUERY_PARAM)
        .send({
          ...formSubmission,
        });
