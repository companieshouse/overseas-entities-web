...
describe("Remove cannot use this service page", () => {
  test(`renders the ${config.REMOVE_CANNOT_USE_PAGE} page`, async () => {
    ...
    expect(resp.text).toContain(`${config.SIGN_OUT_PAGE}?page=${config.REMOVE_SECTION}${config.REMOVE_CANNOT_USE_PAGE}&amp;${config.JOURNEY_QUERY_PARAM}=${config.JourneyType.remove}&amp;${config.PREVIOUS_PAGE_QUERY_PARAM}=${config.REMOVE_SOLD_ALL_LAND_FILTER_PAGE}`);
