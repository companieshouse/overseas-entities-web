...
describe("service availability middleware tests", () => {
  ...
  test(`should redirect to signin page after next middleware (authentication) when req.path is equal ${STARTING_NEW_URL} `, async () => {
    mockIsActiveFeature
  ...
  test(`should redirect to signin page after next middleware (authentication) when req.path ends with '/${RESUME}'`, async () => {
    mockIsActiveFeature
      .mockReturnValueOnce(false)
      .mockReturnValueOnce(true)
      .mockReturnValueOnce(true);
