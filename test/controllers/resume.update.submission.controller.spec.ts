...
import {
  ...
  UPDATE_AN_OVERSEAS_ENTITY_URL, SECURE_UPDATE_FILTER_WITH_PARAMS_URL
} from "../../src/config";
...
describe("Resume update submission controller", () => {
  ...
  test(`Redirect to ${SECURE_UPDATE_FILTER_WITH_PARAMS_URL} page and map trustees address when REDIS_flag is set to ON`, async () => {
    mockIsActiveFeature.mockReturnValue(true);
    mockGetOverseasEntity.mockReturnValueOnce(appData);
    ...
    expect(data.update.review_trusts[0].INDIVIDUALS).toEqual([
      expect.objectContaining(returnData)
    ]);
  });

  test(`Redirect to ${SECURE_UPDATE_FILTER_URL} page and map trustees address when REDIS_flag is set to OFF`, async () => {
    mockIsActiveFeature.mockReturnValue(false);
    mockGetOverseasEntity.mockReturnValueOnce(appData);
