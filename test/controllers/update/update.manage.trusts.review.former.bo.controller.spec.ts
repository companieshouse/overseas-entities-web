...
import { UPDATE_MANAGE_TRUSTS_REVIEW_FORMER_BO_URL, UPDATE_MANAGE_TRUSTS_REVIEW_INDIVIDUALS_URL, UPDATE_MANAGE_TRUSTS_REVIEW_THE_TRUST_URL } from '../../../src/config';
...
import { manageTrustsReviewFormerBOsGuard } from '../../../src/middleware/navigation/update/manage.trusts.middleware';
...
const mockManageTrustsReviewFormerBOsGuard = manageTrustsReviewFormerBOsGuard as jest.Mock;
mockManageTrustsReviewFormerBOsGuard.mockImplementation((req: Request, res: Response, next: NextFunction) => next());
...
describe('Update - Manage Trusts - Review former beneficial owners', () => {
  ...
  describe('GET tests', () => {
    test('when feature flag is on, page is returned', async () => {
      ...
      expect(resp.text).toContain("Review former beneficial owners");
  ...
  describe('POST tests', () => {
    test('when feature flag is on, redirect to review individuals page', async () => {
      mockIsActiveFeature.mockReturnValue(true);
