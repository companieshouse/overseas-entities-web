...
export const get = (req: Request, res: Response, next: NextFunction) => {
  try {
    logger.debugRequest(req, `GET ${config.UPDATE_BENEFICIAL_OWNER_STATEMENTS_PAGE}`);

    const appData: ApplicationData = getApplicationData(req.session);
    return res.render(config.UPDATE_BENEFICIAL_OWNER_STATEMENTS_PAGE, {
      backLinkUrl: config.ENTITY_URL,
      templateName: config.UPDATE_BENEFICIAL_OWNER_STATEMENTS_PAGE,
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
  try {
    logger.debugRequest(req, `POST ${config.UPDATE_BENEFICIAL_OWNER_STATEMENTS_PAGE}`);
