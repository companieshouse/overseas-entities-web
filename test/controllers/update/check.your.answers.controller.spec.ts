...
import {
  ...
  CHECK_YOUR_ANSWERS_PAGE_RP_NO_BENEFICIARY_STATEMENTS_SUB_TEXT, UPDATE_CHANGE_LINK_RP_BO_STATEMENT
...
describe("CHECK YOUR ANSWERS controller", () => {
  ...
  describe("GET tests", () => {
    ...
    ])(`renders the ${UPDATE_CHECK_YOUR_ANSWERS_PAGE} page with relevant period statement not selected detail section %s`, async (_journeyType, mockAppData) => {
      mockGetApplicationData.mockReturnValue(mockAppData);
      const resp = await request(app).get(UPDATE_CHECK_YOUR_ANSWERS_URL);
