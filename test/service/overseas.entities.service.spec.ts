...
const mockUnauthorisedResponseHandler = unauthorisedResponseHandler as jest.Mock;
...
describe(`Overseas Entity Service test suite`, () => {
  ...
  test(`createOverseasEntity should responde with created httpStatusCode`, async () => {
    mockUnauthorisedResponseHandler.mockReturnValueOnce( { httpStatusCode: 201, resource: { id: OVERSEAS_ENTITY_ID } } );
    ...
    expect(mockUnauthorisedResponseHandler).toBeCalledWith("postOverseasEntity", req, session, TRANSACTION_ID, APPLICATION_DATA_MOCK, false);
  });
  ...
  test(`createOverseasEntity should responde with UNAUTHORISED error message`, async () => {
    ...
    mockUnauthorisedResponseHandler.mockReturnValueOnce( mockData );
...
describe(`Update Overseas Entity Service test suite`, () => {
  ...
  test(`updateOverseasEntity should responde with created httpStatusCode`, async () => {
    ...
    mockGetApplicationData.mockReturnValueOnce( APPLICATION_DATA_MOCK );
    mockUnauthorisedResponseHandler.mockResolvedValueOnce( mockResponse);
    ...
    expect(mockUnauthorisedResponseHandler).toBeCalledWith("putOverseasEntity", req, session, TRANSACTION_ID, OVERSEAS_ENTITY_ID, APPLICATION_DATA_MOCK);
  });
