...
describe("Remove registered owner controller", () => {
  ...
  describe("GET tests", () => {
    ...
    test(`renders the ${config.REMOVE_IS_ENTITY_REGISTERED_OWNER_PAGE} page when REDIS_flag is set to OFF`, async () => {
    ...
    test(`renders the ${config.REMOVE_IS_ENTITY_REGISTERED_OWNER_PAGE} page when REDIS_flag is set to ON`, async () => {
      mockGetApplicationData.mockReturnValueOnce({});
  ...
  describe("POST tests", () => {
    ...
    test(`redirects to the ${config.REMOVE_CANNOT_USE_URL} page when yes is selected when the REDIS_flag is set to OFF`, async () => {
    ...
    test(`redirects to the ${config.REMOVE_CANNOT_USE_URL} page when yes is selected when the REDIS_flag is set to ON`, async () => {
    ...
    test(`redirects to the ${config.REMOVE_CANNOT_USE_URL} page when yes is selected and REDIS_removal flag is set to ON`, async () => {
      const resp = await request(app)
        .post(`${config.REMOVE_IS_ENTITY_REGISTERED_OWNER_URL}${config.JOURNEY_REMOVE_QUERY_PARAM}`)
        .send({ is_listed_as_property_owner: config.BUTTON_OPTION_YES });
      mockIsActiveFeature.mockReturnValue(true);
      ...
      expect(populatedRemoveObject[IsListedAsPropertyOwnerKey]).toEqual(config.BUTTON_OPTION_YES);
      expect(mockSetApplicationData.mock.calls[0][2]).toEqual(RemoveKey);
      expect(mockSaveDataToCookie).not.toHaveBeenCalled();
    });
